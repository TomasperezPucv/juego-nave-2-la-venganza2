...
var textbox_x = camera_get_view_x(view_camera[0])+110;
var textbox_y = camera_get_view_y(view_camera[0])+230;
...
if(setup == false){
	...
	//loopear por las paginas
	//page_number = array_length(text);
	for(var p =0; p<page_number; p++){
		//buscar cuantos caracteres tiene cada pagina y guardar el numero en el arrglo text_length
		text_length[p] = string_length(text[p]);
		
		//siempre revisa estos valores al trabajar con ventanas de distintos tamaño
		
		//obtener la posicion x para el text box
		//personaje a laizquierda
			text_x_offset[p] =80;
			...
			//no tener caracteres (centralizar el textbox)
		...
		//setting individual character and finding wjere the line should break
		for(var c=0; c< text_length[p];c++){
			var _char_pos =c+1;
			
			
			//guardar los caracteres individuales en el arrreglo char
			char[c,p] =string_char_at(text[p],_char_pos);
			
			//het current width of the line
		...
		//getting each character coordenate
		for(var c=0; c< text_length[p];c++){
			var _char_pos = c+1;
...
if(text_pause_timer<=0){
	...
	if(draw_char < text_length[page]){
		...
		if(_check_char == "." || _check_char == "?"){
			...
			if(!audio_is_playing(snd[page])){audio_play_sound(snd[page], 0,false);}
...
if((draw_char == text_length[page])&&(page == page_number-1)){
	...
	for(var op =0 ; op< option_number; op++){
		...
		//dibujar flechita
		if(option_pos == op){
			draw_sprite(spr_flecha_seleccion, 0, _txtb_x, _txtb_y -_op_space*option_number + _op_space*op)
		}
		
		
		//aca se escriben las opciones
...
for(var c=0; c<draw_char ; c++){
	// cositas especiales
	//texto wavy o nose como se dice salu2
	var _float_y =0
